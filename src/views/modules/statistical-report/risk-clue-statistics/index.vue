<template>
  <div>
    <el-form :inline="true" :model="params" label-width="80px">
      ...
      <el-form-item>
        <el-button type="primary" @click="getDataList(1)" v-if="isAuth('biz:commonclue:getCommonClue')">查询</el-button>
      </el-form-item>
      <el-form-item>
        <el-button @click="resDataList"> 重置</el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="importTemplate()" v-if="isAuth('biz:exportCommonClue:export')">导出</el-button>
      </el-form-item>
    </el-form>
...
import { getQueryViewRiskClues, exportViewRiskClues} from '@/api/statistical-report/statistics-of-rectification-problems/index'
...
export default {
  ...
  data () {
    return {
      ...
      table: {
        ...
        //     formatter: (row) => {
        //       let a = ''
        //       this.problemDiscoveryArr.forEach((item) => {
        //         if (row.way !== null) {
        //           if (item.dictValue === row.way.toString()) {
        //             a = item.dictLabel
        //           }
        //         }
        //       })
  ...
  methods: {
    getStatus () {
      this.problemDiscoveryArr = this.getDictDataList('problem_discovery')
    },

      // 导出
    importTemplate () {
      let params = { ...this.params }
      exportViewRiskClues(params).then((res) => {
        if (res) {
          const aLink = document.createElement('a')
          let blob = new Blob([res.data], {type: 'application/vnd.ms-excel'})
          aLink.href = URL.createObjectURL(blob)
          aLink.setAttribute('download', '风险线索统计' + '.xlsx') // 设置下载文件名称
          aLink.click()
    ...
    pickerChange () {
      if (this.dates.length) {
        this.params.beginTime = this.dates[0]
    ...
    getDataList (page) {
      ...
      getQueryViewRiskClues(params).then((res) => {
