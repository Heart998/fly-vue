<template>
  <div>
    ...
    <el-form :model="forms" :inline="true" ref="forms" label-width="150px" class="problemDisabled">
      <el-row>
        <h3 class="form_h3_title">
          <div class="line_type_shu"></div>任务信息
        </h3>
        <el-form-item label="风险排查时间:" prop="day">
          <el-date-picker :class="{noborder: problemDisabled}" v-model="forms.day" :picker-options="pickerOptions" :disabled="problemDisabled" value-format="yyyy-MM-dd" type="date" style="width:240px"
            placeholder="选择日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="培训课件数量:" prop="pxNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.pxNum" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="宣传资料数量:" prop="xcNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.xcNum" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="企划方案数量:" prop="qhNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.qhNum" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="产说会课件数量:" prop="cshNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.cshNum" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="微信数量:" prop="wxNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.wxNum" style="width:240px"></el-input>
        </el-form-item>

        <el-form-item label="短信数量:" prop="dxNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.dxNum" style="width:240px"></el-input>
        </el-form-item>

        <el-form-item label="朋友圈数量:" prop="pyqNum">

          <el-input :class="{noborder: problemDisabled}" type="number" v-model="forms.pyqNum" style="width:240px"></el-input>
        </el-form-item>
        <el-col :span="24">
        ...
        <el-col :span="24">
          <el-form-item label="其他:" class="flex-box children_flex" prop="other">

            <el-input :class="{noborder: problemDisabled}" type="textarea" v-model="forms.other"></el-input>
          </el-form-item>
        </el-col>
        <div class="clear"></div>
        <h3 class="form_h3_title">
...
export default {
  ...
  data() {
    return {
      ...
      dataRule: {
        day: [
          { required: true, message: '风险排查时间不能为空', trigger: 'blur' }
        ],
        pxNum: [
          { required: true, message: '培训课件数量不能为空', trigger: 'blur' }
        ],
        xcNum: [
          { required: true, message: '宣传资料课件数量不能为空', trigger: 'blur' }
        ],
        qhNum: [
          { required: true, message: '企划方案数量不能为空', trigger: 'blur' }
        ],
        cshNum: [
          { required: true, message: '产说会课件数量不能为空', trigger: 'blur' }
        ],
        wxNum: [
      ...
      this.$refs.forms.validate((validate) => {
        if (validate) {
          this.$confirm('是否暂存数据？', '提示', {
            ...
            // delete params.id
            staingContactRisk(params).then((res) => {
                  var data = res.data
        ...
        if (vals) {
          this.$confirm('任务提交后将不能修改，是否继续提交？', '提示', {
            ...
            .then(() => {
              ...
              contactriskEditFn(params).then((res) => {
                var data = res.data
                this.btnloading = false
                if (data.code === 0 && data.msg === 'success') {
      ...
      this.$refs.forms.validate((validate) => {
        if (validate) {
          this.$confirm('任务提交后将不能修改，是否继续提交？', '提示', {
            ...
            params.taskId = params.taskId
            // delete params.id
            contactriskSubmitFn(params).then((res) => {
              var data = res.data
              this.btnloading = false
              if (data.code === 0 && data.msg === 'success') {
                this.$message.success('提交成功')
