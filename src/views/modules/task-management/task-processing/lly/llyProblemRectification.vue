<template>
  <div>
    ...
    <h3 class="form_h3_title">
      <div class="line_type_shu"></div>基本信息
    </h3>
    <taskBaseInfo v-bind="forms"></taskBaseInfo>

    <h3 class="form_h3_title">
      <div class="line_type_shu"></div>关联问题
    </h3>
    ...
    <el-form :model="visForms" :inline="true" label-width="170px" ref="visForms" :rules="dataRule" class="problemDisabled">
      <el-row>
        <h3 class="form_h3_title">
        ...
        <el-form-item label="整改主责条线:" prop="reformLine">
          <el-input :disabled="problemDisabled" :class="{noborder: problemDisabled}" v-model="visForms.reformLine" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="整改主责岗位:" prop="reformPost">
          <el-input :disabled="problemDisabled" :class="{noborder: problemDisabled}" v-model="visForms.reformPost" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="整改负责人:" prop="reformUser">
          <el-input :disabled="problemDisabled" :class="{noborder: problemDisabled}" v-model="visForms.reformUser" style="width:240px"></el-input>
        </el-form-item>
        <el-form-item label="配合整改部门:" prop="reformDept">
          <el-input :disabled="problemDisabled" :class="{noborder: problemDisabled}" v-model="visForms.reformDept" style="width:240px"></el-input>
        </el-form-item>
        ...
        <el-col :span="24">
          <el-form-item label="整改结果:" class="flex-box children_flex" prop="rectificationResults">
...
export default {
  ...
  data () {
    return {
      ...
      dataRule: {
        ...
        reformTime: [
          { required: true, message: '计划完成整改时间不能为空', trigger: 'blur' }
        ],
        rectificationStatus: [{
          required: true,
          message: '整改状态',
        ...
        punishSales: [
          { required: true, message: '销售人员问责措施不能为空', trigger: 'blur' }
        ],
        punishManager: [
          { required: true, message: '管理人员问责措施不能为空', trigger: 'blur' }
        ]
      }
    }
  },
  methods: {
    ...
    onSubmit () {
      this.$refs.visForms.validate((validate) => {
        if (validate) {
          this.$confirm('任务提交后将不能修改，是否继续提交？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
      ...
      if (this.taskParams.id === '' && this.taskParams.type === '') {
        ...
        if (this.taskParams.type === 'chuli') {
          if (this.taskParams.islook === 'yes') {
            ...
            this.forms.fileList2.uploadbtnshow = true
