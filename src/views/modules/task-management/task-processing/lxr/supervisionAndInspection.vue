<template>
  <div>
    ...
    <el-form :model="forms" :inline="true" label-width="150px" class="problemDisabled" ref="forms" :rules="dataRule">
      <h3 class="form_h3_title">
        <div class="line_type_shu"></div>任务信息
      </h3>
      <el-form-item label="督导检查项目:" prop="project">
        <el-input :class="{noborder: problemDisabled}" :disabled="problemDisabled" v-model="forms.project" style="width:240px"></el-input>
      </el-form-item>
      <el-form-item label="督查单位:" prop="company">
        <el-input :class="{noborder: problemDisabled}" :disabled="problemDisabled" v-model="forms.company" style="width:240px"></el-input>
      </el-form-item>
      <el-form-item label="督查数量:" prop="number">
        <el-input :class="{noborder: problemDisabled}" type="number" :disabled="problemDisabled" v-model="forms.number" style="width:240px">
          <template slot="append">次</template>
        </el-input>
      </el-form-item>
      <el-form-item label="督查比例:" prop="ratio">
        <el-input :class="{noborder: problemDisabled}" type="number" :disabled="problemDisabled" v-model="forms.ratio" style="width:240px">
          <template slot="append">%</template>
        </el-input>
      </el-form-item>
      <el-form-item label="督查工作方案:" prop="programme">
        <el-select v-model="forms.programme" style="width:240px;" :class="{noborder: problemDisabled}" type="number" :disabled="problemDisabled">
          <el-option :label="item.label" :value="item.id" v-for="(item, index) in checkTypeArr" :key="index"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="督查工作底稿:" prop="draft">
        <el-select v-model="forms.draft" style="width:240px;" :class="{noborder: problemDisabled}" type="number" :disabled="problemDisabled">
          <el-option :label="item.label" :value="item.id" v-for="(item, index) in checkTypeArr" :key="index"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="督查工作报告:" prop="report">
        <el-select v-model="forms.report" style="width:240px;" :class="{noborder: problemDisabled}" type="number" :disabled="problemDisabled">
          <el-option :label="item.label" :value="item.id" v-for="(item, index) in checkTypeArr" :key="index"></el-option>
        </el-select>
      </el-form-item>
      <el-col :span="24">
      ...
      <h3 class="form_h3_title">
        <div class="line_type_shu"></div>督导检查发现问题明细
      </h3>
...
import {
  linkmaninspectSubmitFn,
  linkmaninspectEditFn,
  getFormsHx,
  getFMQDDM,
  getCheckCorpInfo,
  getFMQDDMCODE
...
export default {
  ...
  data () {
    return {
      ...
      dataRule: {
        project: [
          { required: true, message: '督导检查项目不能为空', trigger: 'blur' }
        ],
        company: [
          { required: true, message: '督查单位不能为空', trigger: 'blur' }
        ],
        number: [
          { required: true, message: '督查数量不能为空', trigger: 'blur' }
        ],
        ratio: [
          { required: true, message: '督查比例不能为空', trigger: 'blur' }
        ],
        fileList2: [
  ...
  methods: {
    ...
    onEdit () {
      if (this.forms.taskFiles == '' || this.forms.taskFiles == undefined) {
        this.$message({
          type: 'info',
          message: '请上传凭证'
        })
        ...
        if (validate) {
          this.$confirm('任务提交后将不能修改，是否继续提交？', '提示', {
            ...
            .then(() => {
              ...
              linkmaninspectEditFn(params).then((res) => {
                var data = res.data
      ...
      this.$refs.forms.validate((validate) => {
          ...
          this.$confirm('任务提交后将不能修改，是否继续提交？', '提示', {
            ...
            .then(() => {
              ...
              delete params.id
              linkmaninspectSubmitFn(params).then((res) => {
                var data = res.data
                this.btnloading = false
                if (data.code === 0 && data.msg === 'success') {
                  this.$message.success('提交成功')
